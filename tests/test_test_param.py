...
def test_weight_and_max_score_cannot_be_specified_both() -> None:
    with pytest.raises(ValueError, match=re.escape("Cannot specify both gap_max_score and gap_weight.")):
...
def test_test_params_gap_kwargs_length_matching() -> None:
    with pytest.raises(
        ValueError,
        match=re.escape("all gap_ keyword args must have the same length as the test cases,"),
    ):
    ...
    with pytest.raises(
        ...
        test_cases.params([1], [2], [3], gap_hidden=[True, False, False], gap_max_score=[1, 2])
...
def test_test_params_params(args: Sequence[Any], kwargs: Dict, results: Sequence[param]) -> None:
